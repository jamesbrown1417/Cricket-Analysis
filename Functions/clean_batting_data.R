...
clean_batting_data <- function(x)
{
  ...
  vars <- colnames(x)
  if(career)
    varorder <- c("Player","Country","Start","End","Matches","Innings","NotOuts","Runs","HighScore","HighScoreNotOut",
                  "Average","BallsFaced","StrikeRate","Hundreds","Fifties","Ducks","Fours","Sixes")
  else
    varorder <- c("Date","Player", "Country", "Runs", "NotOut", "Minutes", "BallsFaced", "Fours", "Sixes",
