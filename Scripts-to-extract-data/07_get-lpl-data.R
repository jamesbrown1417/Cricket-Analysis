...
lpl_match_data$venue <- clean_venue_names(lpl_match_data$venue)
...
get_match_innings_data <- function(match_id_number) {
  # Match dataset
  match_df <- lpl_ball_by_ball_data |> filter(match_id == match_id_number)
  
  # Match Details
  match_details <- lpl_match_data |> mutate(match_id = as.integer(match_id)) |>  filter(match_id == match_id_number)
...
lpl_batting_innings_level <-
  lpl_batting_innings_level |>
  left_join(lpl_match_data |> mutate(match_id = as.integer(match_id)),
            by = "match_id")
...
# Output Data
write_rds(lpl_match_innings_data, "Data/T20s/LPL/lpl_match_innings_data.rds")
...
# Output Data
write_rds(lpl_match_first_over_data, "Data/T20s/LPL/lpl_first_over_data.rds")
...
# Output Data
write_rds(lpl_batting_innings_level, "Data/T20s/LPL/lpl_batting_innings_level.rds")
...
# Output Data
write_rds(lpl_bowling_innings_level, "Data/T20s/LPL/lpl_bowling_innings_level.rds")
