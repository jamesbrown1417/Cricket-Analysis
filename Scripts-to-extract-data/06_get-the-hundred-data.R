...
# Match Data--------------------------------------------------------------------
the_hundred_match_data <-
  fetch_cricsheet(type = "match",
                  competition = "hnd",
                  gender = "male")

# Player Data-------------------------------------------------------------------
the_hundred_player_data <-
  fetch_cricsheet(type = "player",
                  competition = "hnd",
                  gender = "male")

# Ball by Ball Data-------------------------------------------------------------
the_hundred_ball_by_ball_data <-
...
# Define the function to clean stadium names based on predefined rules
clean_venue_names <- function(stadiums) {
  stadiums_cleaned <- sapply(stadiums, function(stadium) {
    case_when(
      str_detect(stadium, "Chinnaswamy|Bengaluru") ~ "M Chinnaswamy Stadium, Bengaluru",
      str_detect(stadium, "Punjab Cricket Association|Mohali") ~ "Punjab Cricket Association Stadium, Mohali",
      str_detect(stadium, "Feroz Shah Kotla|Arun Jaitley") ~ "Arun Jaitley Stadium, Delhi",
      str_detect(stadium, "Eden Gardens") ~ "Eden Gardens, Kolkata",
      str_detect(stadium, "Wankhede") ~ "Wankhede Stadium, Mumbai",
      str_detect(stadium, "Sawai Mansingh") ~ "Sawai Mansingh Stadium, Jaipur",
      str_detect(stadium, "Rajiv Gandhi International|Uppal") ~ "Rajiv Gandhi International Stadium, Uppal, Hyderabad",
      str_detect(stadium, "MA Chidambaram|Chepauk") ~ "MA Chidambaram Stadium, Chepauk, Chennai",
      str_detect(stadium, "Dr DY Patil") ~ "Dr DY Patil Sports Academy, Mumbai",
      str_detect(stadium, "Sardar Patel|Motera|Narendra Modi") ~ "Narendra Modi Stadium, Ahmedabad",
      str_detect(stadium, "Himachal Pradesh Cricket Association") ~ "Himachal Pradesh Cricket Association Stadium, Dharamsala",
      str_detect(stadium, "Dr. Y.S. Rajasekhara Reddy|ACA-VDCA|Visakhapatnam") ~ "Dr. Y.S. Rajasekhara Reddy ACA-VDCA Cricket Stadium, Visakhapatnam",
      str_detect(stadium, "Maharashtra Cricket Association") ~ "Maharashtra Cricket Association Stadium, Pune",
      TRUE ~ stadium  # Default case to return the original if no matches found
    )
  })
  
  return(stadiums_cleaned)
}

# Apply the function to the venue data
the_hundred_match_data$venue <- clean_venue_names(the_hundred_match_data$venue)
...
get_match_innings_data <- function(match_id_number) {
  # Match dataset
  match_df <- the_hundred_ball_by_ball_data |> filter(match_id == match_id_number)
  
  # Match Details
  match_details <- the_hundred_match_data |> mutate(match_id = as.integer(match_id)) |>  filter(match_id == match_id_number)
...
the_hundred_match_innings_data <-
  the_hundred_match_data |> 
  distinct(match_id) |> 
...
the_hundred_match_first_over_data <-
  the_hundred_match_data |> 
  distinct(match_id) |> 
  rename(match_id_number = match_id) |>
  mutate(match_id_number = as.integer(match_id_number)) |>
...
the_hundred_batting_innings_level <-
  the_hundred_ball_by_ball_data |>
...
the_hundred_batting_innings_level <-
  the_hundred_batting_innings_level |>
  left_join(the_hundred_match_data |> mutate(match_id = as.integer(match_id)),
            by = "match_id")
...
the_hundred_batting_innings_level <-
  the_hundred_batting_innings_level |>
  left_join(the_hundred_batting_order,
...
# Get tibble of match_id_number and player_name to iterate over-----------------
the_hundred_bowling_innings_level <-
  the_hundred_ball_by_ball_data |>
...
# Output Data
write_rds(the_hundred_match_innings_data, "Data/T20s/The Hundred/the_hundred_match_innings_data.rds")
...
# Output Data
write_rds(the_hundred_match_first_over_data, "Data/T20s/The Hundred/the_hundred_first_over_data.rds")
...
# Output Data
write_rds(the_hundred_batting_innings_level, "Data/T20s/The Hundred/the_hundred_batting_innings_level.rds")
...
# Output Data
write_rds(the_hundred_bowling_innings_level, "Data/T20s/The Hundred/the_hundred_bowling_innings_level.rds")
