...
t20_mlc_match_data$venue <- clean_venue_names(t20_mlc_match_data$venue)
...
get_match_innings_data <- function(match_id_number) {
  # Match dataset
  match_df <- t20_mlc_ball_by_ball_data |> filter(match_id == match_id_number)
  
  # Match Details
  match_details <- t20_mlc_match_data |> mutate(match_id = as.integer(match_id)) |>  filter(match_id == match_id_number)
...
t20_mlc_batting_innings_level <-
  t20_mlc_batting_innings_level |>
  left_join(t20_mlc_match_data |> mutate(match_id = as.integer(match_id)),
            by = "match_id")
...
t20_mlc_batting_innings_level <-
  t20_mlc_batting_innings_level |>
  left_join(t20_mlc_batting_order,
...
# Output Data
write_rds(t20_mlc_match_innings_data, "Data/T20s/Major League Cricket/t20_mlc_match_innings_data.rds")
...
# Output Data
write_rds(t20_mlc_batting_innings_level, "Data/T20s/Major League Cricket/t20_mlc_batting_innings_level.rds")
...
# Output Data
write_rds(t20_mlc_bowling_innings_level, "Data/T20s/Major League Cricket/t20_mlc_bowling_innings_level.rds")