...
cpl_match_data$venue <- clean_venue_names(cpl_match_data$venue)
...
get_match_innings_data <- function(match_id_number) {
  # Match dataset
  match_df <- cpl_ball_by_ball_data |> filter(match_id == match_id_number)
  
  # Match Details
  match_details <- cpl_match_data |> mutate(match_id = as.integer(match_id)) |>  filter(match_id == match_id_number)
...
cpl_batting_innings_level <-
  cpl_batting_innings_level |>
  left_join(cpl_match_data |> mutate(match_id = as.integer(match_id)),
            by = "match_id")
...
# Output Data
write_rds(cpl_match_innings_data, "Data/T20s/CPL/cpl_match_innings_data.rds")
...
# Output Data
write_rds(cpl_match_first_over_data, "Data/T20s/CPL/cpl_first_over_data.rds")
...
# Output Data
write_rds(cpl_batting_innings_level, "Data/T20s/CPL/cpl_batting_innings_level.rds")
...
# Output Data
write_rds(cpl_bowling_innings_level, "Data/T20s/CPL/cpl_bowling_innings_level.rds")
