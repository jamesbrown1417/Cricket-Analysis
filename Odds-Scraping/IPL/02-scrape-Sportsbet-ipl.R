...
pad_na <- function(x, n) {
  if (length(x) < n) c(x, rep(NA, n - length(x))) else x
}
...
# Function to fix team names for Sportsbet IPL
...
main_markets_function <- function() {
  ...
  # Write to csv
  write_csv(sportsbet_h2h, "Data/T20s/IPL/scraped_odds/sportsbet_h2h.csv")
...
player_props_function <- function() {
  ...
  milestones_links <-
    glue(
      "https://www.sportsbet.com.au/apigw/sportsbook-sports/Sportsbook/Sports/Events/{match_ids}/MarketGroupings/903/Markets"
    )
    ...
    for (market in sb_response) {
      for (selection in market$selections) {
        ...
        if (is.null(selection$price$winPrice)) {
          selection$selection$price$winPrice = NA
          prop_market_price = c(prop_market_price, selection$selection$price$winPrice)
        }
        else {
  ...
  player_runs_combined <-
    ...
    mutate(
      player_name = case_when(
        str_detect(player_name, "T.?.? Rogers ?\\(Stars\\)") ~ "Tom Rogers",
        str_detect(player_name, "K Jennings") ~ "Keaton Jennings",
        .default = player_name
      )
    ) |>
    mutate(agency = "Sportsbet")
  ...
  write_csv(match_top_run_scorer,
            "Data/T20s/IPL/scraped_odds/sportsbet_match_top_run_scorer.csv")
  
  write_csv(player_runs_all,
            "Data/T20s/IPL/scraped_odds/sportsbet_player_runs.csv")
  ...
  player_milestones_data <-
    map(milestones_links, safe_read_prop_url)
  ...
  # Write to csv----------------------------------------------------------------
  write_csv(player_fifties_combined,
            "Data/T20s/IPL/scraped_odds/sportsbet_player_fifties.csv")
  ...
  write_csv(
    player_wickets_alternative_lines,
    "Data/T20s/IPL/scraped_odds/sportsbet_player_wickets.csv"
  )
  
  write_csv(
    top_team_wicket_taker,
    "Data/T20s/IPL/scraped_odds/sportsbet_top_team_wicket_taker.csv"
  )
  ...
  team_totals <-
    ...
    if(nrow(team_totals) == 0) {
      team_totals <- tibble(url = "", prop_market_name = "", selection_name_prop = "", prop_market_price = "", handicap = "")
    }
  ...
  write_csv(team_totals_overs,
            "Data/T20s/IPL/scraped_odds/sportsbet_team_totals_overs.csv")
  
  write_csv(team_totals_six,
            "Data/T20s/IPL/scraped_odds/sportsbet_team_totals_six_overs.csv")
  
  write_csv(team_totals_ten,
            "Data/T20s/IPL/scraped_odds/sportsbet_team_totals_ten_overs.csv")
  
  write_csv(team_fours,
            "Data/T20s/IPL/scraped_odds/sportsbet_team_fours.csv")
  
  write_csv(team_sixes,
            "Data/T20s/IPL/scraped_odds/sportsbet_team_sixes.csv")
  
  write_csv(match_sixes,
            "Data/T20s/IPL/scraped_odds/sportsbet_match_sixes.csv")
  
  write_csv(match_fours,
            "Data/T20s/IPL/scraped_odds/sportsbet_match_fours.csv")
  ...
  write_csv(first_over_runs_all,
            "Data/T20s/IPL/scraped_odds/sportsbet_first_over_runs.csv")
  ...
  if(nrow(fall_of_wicket_markets) == 0) {
    fall_of_wicket_markets <- tibble(url = "", prop_market_name = "", selection_name_prop = "", prop_market_price = "", handicap = "")
  }
  ...
  write_csv(fall_of_wicket_all,
            "Data/T20s/IPL/scraped_odds/sportsbet_runs_at_first_wicket.csv")
  ...
  write_csv(first_over_runs,
            "Data/T20s/IPL/scraped_odds/sportsbet_first_over_runs_first_innings.csv")
  write_csv(first_dismissal_data,
            "Data/T20s/IPL/scraped_odds/sportsbet_first_dismissal_first_innings.csv")
  write_csv(first_wicket_runs,
            "Data/T20s/IPL/scraped_odds/sportsbet_runs_at_first_wicket_first_innings.csv")
  ...
  player_boundaries_combined |>
    ...
    write_csv("Data/T20s/IPL/scraped_odds/sportsbet_player_boundaries.csv")
}
