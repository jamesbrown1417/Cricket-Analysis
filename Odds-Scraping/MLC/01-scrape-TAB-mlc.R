...
highest_opening_partnership <-
  ...
  mutate(prop_name = fix_team_names_mlc(prop_name)) |>
...
# Filter to most match sixes markets
most_match_sixes <-
  all_tab_markets |>
  filter(market_name == "Most Sixes 3 Way") |> 
  separate(match, into = c("home_team", "away_team"), sep = " v ", remove = FALSE) |>
  mutate(home_team = fix_team_names_mlc(home_team),
         away_team = fix_team_names_mlc(away_team)) |>
  mutate(prop_name = fix_team_names_mlc(prop_name)) |>
  mutate(match = paste(home_team, "v", away_team)) |> 
  mutate(market_name = "Team To Score Most Sixes") |> 
  select(match, 
         home_team,
         away_team,
         market = market_name,
...
# Write out
most_match_sixes |> 
  write_csv("Data/T20s/Major League Cricket/scraped_odds/tab_most_match_sixes.csv")
