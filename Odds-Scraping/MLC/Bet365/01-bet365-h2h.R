...
get_head_to_head <- function() {
  ...
  scraped_file <- list.files("Odds-Scraping/MLC/Bet365/HTML", full.names = TRUE, pattern = "h2h")[[1]]
  ...
  # Combine together into one table (swap home/away to match other bookmakers)
  bet365_h2h <-
    bind_cols(home_h2h, away_h2h) |>
    transmute(match = paste(away_teams, home_teams, sep = " v "),
              market_name = "Head To Head",
              home_team = away_teams,
              home_win = as.numeric(away_odds),
              away_team = home_teams,
              away_win = as.numeric(home_odds)) |>
...
# Write to csv
write_csv(bet365_h2h, "Data/T20s/Major League Cricket/scraped_odds/bet365_h2h.csv")