...
async def main():
    ...
    async with webdriver.Chrome(options=options) as driver:
        await driver.get('https://www.bet365.com.au/#/AC/B3/C21063435/D720/E1246/F720/')
        ...
        with open(f"Odds-Scraping/MLC/Bet365/HTML/h2h_html.txt", 'w') as f:
            f.write(body_html)
        ...
        # Write URL as a csv
        match_url_list = '\n'.join(match_url_list)
        with open(f"Odds-Scraping/MLC/Bet365/match_urls.csv", 'w') as f:
           f.write(match_url_list)
           
        player_url_list = '\n'.join(player_url_list)
        with open(f"Odds-Scraping/MLC/Bet365/player_urls.csv", 'w') as f:
           f.write(player_url_list)
           
        first_over_url_list = '\n'.join(first_over_url_list)
        with open(f"Odds-Scraping/MLC/Bet365/first_over_urls.csv", 'w') as f:
           f.write(first_over_url_list)
           
        first_innings_url_list = '\n'.join(first_innings_url_list)
        with open(f"Odds-Scraping/MLC/Bet365/first_innings_urls.csv", 'w') as f:
           f.write(first_innings_url_list)
           
        team_url_list = '\n'.join(team_url_list)
        with open(f"Odds-Scraping/MLC/Bet365/team_urls.csv", 'w') as f:
