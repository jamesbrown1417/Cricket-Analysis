# LPL (Lanka Premier League) Sportsbet Scraper
# ==============================================
# Uses the centralized scrape_sportsbet function from Functions/

source("Functions/scrape_sportsbet.R")

# LPL-specific player name fixes
fix_player_names_lpl <- function(player_name_vector) {
  case_when(
...
scrape_sportsbet(
  ...
  output_dir = "Data/T20s/LPL/scraped_odds",
  competition_name = "LPL",
  fix_player_names = fix_player_names_lpl,
  use_live_html = FALSE
)
